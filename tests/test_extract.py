...
def mock_guardian_api_multiple_responses():
    ...
    return {"response": {"status": "ok",
                         ...
                         "pages": 146,
                         "orderBy": "newest",
                         "results": [{"id": "world/2025/oct/13/palestinian-prisoners-return-home",
                                      "type": "article",
                                      "sectionId": "world",
                                      ...
                                      "blocks": {"body": [{"id": "68ed05938f08f99dceddd8d0",
                                     ...
                                     {"id": "world/2025/oct/13/why-us-china-trade-war-restarted-tariffs",
                                      ...
                                      "blocks": {"body": [{"id": "68eccde88f085c374050c5cf",
...
@patch('src.extract.requests.get')
def test_returns_required_fields_multiple_results(
        mock_requests, mock_guardian_api_multiple_responses):
...
@patch.dict('os.environ', {'GUARDIAN_API_KEY': 'test-key'})
@patch('src.extract.requests.get')
def test_lambda_handler_uses_search_term_from_event(
        mock_get, _mock_boto, mock_guardian_api_one_response):
...
def test_lambda_handler_sends_single_article_result_to_sqs(
        mock_get, mock_boto, mock_guardian_api_one_response):
...
@patch.dict('os.environ', {'GUARDIAN_API_KEY': 'test-key', 'SQS_QUEUE_URL': 'test-queue'})
@patch('src.extract.requests.get')
def test_lambda_handler_sends_multiple_articles_to_sqs(
        mock_get, mock_boto, mock_guardian_api_multiple_responses):
    # Arrange
    mock_response = Mock()
    mock_response.json.return_value = mock_guardian_api_multiple_responses
    mock_get.return_value = mock_response
